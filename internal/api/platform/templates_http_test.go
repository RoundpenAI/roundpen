...
func TestHandler_getTemplateBuildStatus_notFound(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/templates/"+uuid.NewString()+"/builds/"+uuid.NewString()+"/status", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
...
func TestHandler_templateCRUD(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/templates/"+created.TemplateID, nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
